				...
				unsigned long iova, int flags, void *data)
{
	...
	for (i = 0; i < size; i++)
		pr_err("[%s: 0x%x\t0x%08X]", dwc3_regs[i].name, dwc3_regs[i].offset,
				readl(dwc->regs + dwc3_regs[i].offset - DWC3_GLOBALS_REGS_START));
	/*
...
static int dwc3_msm_probe(struct platform_device *pdev)
{
	...
	struct dwc3_msm *mdwc;
	struct dwc3	*dwc;
	struct iommu_domain *dwc3_domain;
	struct resource *res;
	int ret = 0, size = 0, i;
	u32 val;
	...
	dwc3_domain = iommu_get_domain_for_dev(&mdwc->dwc3->dev);
	if (dwc3_domain) {
		iommu_set_fault_handler(dwc3_domain, dwc3_msm_smmu_fault_handler, mdwc);
		pr_info("dwc3 iommu fault handler registered");
	} else {
		pr_err("dwc3 iommu domain not available");
	}
